...
(function($) {
    ...
    $.v = $.validity = {
		...
		getValue: function(obj) {
			...
			if (name) {
				if (name.indexOf('[]') != -1 && $(obj).attr('type') == 'checkbox') {
					current_value = [];
					...
					current_value = $('[name="' + name+'"]:checked').val();
				} else {
        ...
        match:function(rule, msg) {
            ...
            return validate(
                ...
                $.isFunction(rule) ?
                    ...
                    function(obj) {
                        return !obj.value.length || rule($.validity.getValue(obj));
                    } :
                    ...
                    function(obj) {
						...
						current_value = $.validity.getValue(obj);
                        /*
                        
                        className = obj.className.split(' ')[0];
                        console.log($('.'+className));
						if (className != '' && $('.'+className).attr('name').indexOf('[]') != -1 && $('.'+className).attr('type') == 'checkbox') {
							current_value = [];
							$('.' + className + ':checked').each(function( ) {
								current_value.push($(this).attr('value'));
							});
						} else if (className != '' && $('.'+className).attr('type') == 'checkbox') {
							current_value = $('.' + className).attr('checked') ? $('.'+className).attr('value') : '';
						} else if (className != '' && $('.'+className).attr('type') == 'radio') {
							current_value = $('.' + className+':checked').val();
						} else {
							current_value = $(obj).val( );
						}
						...
						*/
						...
						if (current_value instanceof Array) {
							...
							for (var i in current_value) {
								result = rule.test(current_value[i]);

								if (result == true)
									return result;
							}
						} else {
							return /*! current_value.length || */rule.test(current_value);
						}
                    },
    ...
    function infer(field) {
        ...
        // Check for title.
        if ($f && $f.attr("title") /*&& $f.attr("title").length*/) {
            ret = $f.attr("title");
        }
...
vs=$.validity.start;ve=$.validity.end;